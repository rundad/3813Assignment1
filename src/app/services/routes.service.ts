...
export class RoutesService {
  ...
  //The login function which sends the login request to the server
  login(email:string){
    return this.http.post<user>(this.url + '/api/auth', {email: email});
  }

  //The function that sends the get users request to the server to get all the users
  getUsers(){
    return this.http.get(this.url + "/getUsers")
  }

  //The function that sends the createuser request to the server for creating a user
  createUser(username: string, email: string){
    return this.http.post(this.url + "/createUser", {username: username, email: email});
  }

  //The function that sends the remove user request to the server for removing a user
  removeUser(username: string){
    return this.http.post(this.url + "/removeUser", {username:username});
  }

  //The function that sends the get groups request to the server for get all the groups
  getGroups(){
    return this.http.get(this.url + "/getGroups")
  }

  //The function that sends the create group request to the server for creating a group
  createGroup(name: string, username:string){
    return this.http.post(this.url + "/createGroup", {name: name, username:username})
  }

  //The function that sends the remove group request to the server for remove a group
  removeGroup(name: string){
    return this.http.post(this.url + "/removeGroup", {name: name})
  }

  //The function that sends the create channel request to the server for creating a channel
  createChannel(group:string, channel:string){
    return this.http.post(this.url + "/createChannel", {group: group, channel: channel})
  }

  //The function that sends the get channel request to the server for get all the channels in the group
  getChannel(group: string){
    return this.http.post(this.url + "/getChannel", {group: group})
  }

  //The function that sends the remove channel request to the server for removing a channel in the group
  removeChannel(group:string, channel: string){
    return this.http.post(this.url + "/removeChannel", {group:group, channel:channel})
  }

  //The function that sends the get current user request to the server for getting the current user details
  getCurrentUser(){
    return this.http.get(this.url + "/getCurrentUser")
  }

  //The function that sends the invite user request to the server for inivting a user to a group
  inviteUser(group:string, username:string){
    return this.http.post(this.url + "/inviteUser", {group: group, username: username})
  }

  //The function that sends the get group users request to the server for getting all the users in the group
  getGroupUsers(group:string){
    return this.http.post(this.url + "/getGroupUsers", {group:group})
  }

  //The function that sends the kick user request to the server for kicking a user from the group
  kickUser(group: string, username:string){
    return this.http.post(this.url + "/kickUser", {group: group, username:username})
  }


  //The function that sends the get group user request to the server for getting users in the group
  gGroupUsers(group:string){
    return this.http.post(this.url + "/gGroupUsers", {group:group})
  }

  //The function that sends the get group channels to the server for getting channels in the group
  getGroupChannel(group:string){
    return this.http.post(this.url + "/getGroupChannel", {group:group})
  }

  //The function used to send the add user channel request to the server for adding a user to a channel in a group
  ...
  //The function thats send the get channel user request to the server for getting users in the channel
  getChannelUsers(group:string, channel:string){
    return this.http.post(this.url + "/getChannelUsers", {group:group, channel:channel})
  }

  //The function that sends the remove user from channel request to the server for removing a user from a channel
  ...
  //The function used to send the create with super request to the server for creating a user with super admin role
  ...
  //The function used to send the giveAssis request to the server for giving group assis role to a user
  giveAssis(group:string, username:string){
    return this.http.post(this.url + "/giveAssis", {group:group, username:username})
  }


  //The function used to send the giveSuper request to the server for giving super admin role to a user
  giveSuper(name:string){
    return this.http.post(this.url + "/giveSuper", {name: name})
  }

  //The function used to send the get channels request to the server for gettings channels in the group
