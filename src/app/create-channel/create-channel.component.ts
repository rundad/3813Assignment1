...
export class CreateChannelComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.username = JSON.parse(sessionStorage.getItem("currentUsername"))
  }
  ...
  createChannel(){
    ...
    if(this.group_channel !== "null" && this.channel_name !== ""){
      this.routeService.createChannel(this.group_channel, this.channel_name, this.username).subscribe(data =>{
        console.log(data)
        if(data === true){
